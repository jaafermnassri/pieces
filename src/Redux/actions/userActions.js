...
export const getCurrentUser = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`http://localhost:5050/api/users/current`, {
      headers: {
...
export const getAllUsers = (role) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`http://localhost:5050/api/users/?role=${role}`,{headers: {
      Authorization: `Bearer ${token}`,
    },});
...
export const deleteUser = (id) => async (dispatch) => {
  const token = localStorage.getItem("token");
  try {
    const res = await axios.delete(`http://localhost:5050/api/users/${id}`,{
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
